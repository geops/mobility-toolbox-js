...
import TrackerLayerWebgl from './TrackerLayerWebgl';
 ...
 */
class TrajservLayerWebgl extends mixin(TrackerLayerWebgl) {
  constructor(options = {}) {
  ...
  updateTrajectories() {
    ...
    this.api
      ...
      .then((featureColl) => {
        if (featureColl) {
          this.trajectories = new GeoJSON().readFeatures(featureColl);
        } else {
          this.trajectories = [];
        }
      });
   ...
   */
  clone(newOptions) {
    return new TrajservLayerWebgl({ ...this.options, ...newOptions });
  }
}

export default TrajservLayerWebgl;
